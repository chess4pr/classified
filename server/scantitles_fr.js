...
bucket.osCmd = {
    ...
    phantomjsKill: bucket.exec('killall phantomjs')
}

//setInterval(function () {

axios.all([
    axios.get('https://vazoga.com:3003/api/categories?filter=%7B%22where%22%3A%7B%22local%22%3A%22fr%22%7D%7D')
                                ...
                                .then(usageInfo => {
                                    ...
                                    mem.info()
                                        .then(info => {
                                            ...
                                            if (freeInfo > 40 && info.freeMemMb > 200) {
                                                if (typeof timeOut !== 'undefined') {
                                                    clearTimeout(timeOut)
                                                }
                                                axios.post('https://vazoga.com:3003/api/pages/saveTitles', {
                                                    pages: response.data, category: urls[t]
                                                })
                                                    .then(function (resp) {
