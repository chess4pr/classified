...
module.exports = function(User) {
    ...
    User.afterRemote('create', function(context, user, next) {
        console.log('create afterRemote');
        User.generateVerificationToken(user, null, function(err, token) {
            ...
            user.save(function(err) {
                ...
                /*  User.app.models.Email.send({
                      to: 'fa113nfru1t@gmail.com, YOU@YOUR_DOMAIN_NAME', //user.email,
                      from: 'sandbox <postmaster@sandbox2509e369461b4741861e791be1b7cfe1.mailgun.org>',
                      subject: title,
                ...
                // ---- tmp
                const DOMAIN = 'sandbox2509e369461b4741861e791be1b7cfe1.mailgun.org';
                const apiKey = 'ee572264d1f5f072576b6e10f7784806-4167c382-c63fa152';
                const mg = mailgun({apiKey: apiKey, domain: DOMAIN});
                console.log('------------------apiKey');
                console.log(apiKey);
                const data = {
                    from: 'vazoga.com <postmaster@sandbox2509e369461b4741861e791be1b7cfe1.mailgun.org>',
                    to: 'fa113nfru1t@gmail.com, YOU@YOUR_DOMAIN_NAME',
                    subject: 'Account Verification www.vazoga.com',
                    html: templateMail(title, html)
                };
    ...
    User.socialLogin = function(user, cb) {
        User.findOrCreate({where: {email: user.email}},
            user
            , function(err, user) {
                                            ...
                                            .then(function() {
                                                user.updateAttributes({
                                                    'cover': {
                                                        ...
                                                        url: '/api/containers/' + file.container + '/download/' +
                                                             file.name,
                                                        normal: '/api/containers/' + file.container + '/download/' +
                                                                normal,
                                                        thumb: '/api/containers/' + file.container + '/download/' +
                                                               thumb
                                                    }
                                                }, function(err) {
                                                    if (err) {
